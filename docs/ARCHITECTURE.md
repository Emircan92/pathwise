...
Project listing metadata
        ↓
Determine completeness from metadata and both stored payloads
```

Each fetch discovers the configured recent ranked set from Riot (50 matches by default). Previously stored matches remain in SQLite; repeated fetches avoid unnecessary Riot requests for complete matches and retry missing resources for matches in the newly discovered set.
...
Pathwise currently persists source payloads and the match metadata needed for listing. Review analysis is calculated from those stored payloads when requested, not persisted.
...
Pathwise stores selected match facts such as queue, played time, duration, champion, result, and role for listing. Reconstruction reads the retained raw payloads rather than a separate normalized event or snapshot store.

Further normalization or persistence of derived analysis should be added only when a concrete need justifies it. Avoid creating relational tables solely to mirror Riot JSON.
...
* rerun analysis on stored matches,
* compare algorithm changes,
* avoid ambiguity about why two results differ.
...
Game reconstruction
      ↓
Time-indexed game state and recorded events
      ↓
Metrics and signals
      ↓
...
```text
0001-game-reconstruction-model.md
0003-review-window-calibration-v2.md
0005-game-knowledge-annotation-policy-v1.md
0006-encounter-detection-v1.md
```
...
# 33. Initial Vertical Slice (Completed)

The first implementation milestone proved the complete data path without sophisticated analysis.

Its target workflow was:
...
This milestone preceded the current review pipeline.

---

# 34. Second Vertical Slice (Completed)

The second milestone used a stored jungle match to build the first analysis path.
...
The known Kha'Zix game became the first analytical fixture.
